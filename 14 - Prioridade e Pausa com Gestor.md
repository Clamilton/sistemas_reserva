...
Pra iniciar uma tarefa (sair de "Aguardando Início" pela primeira vez), o backend verifica:

1. **Entre níveis diferentes:** não dá pra iniciar uma tarefa se existir qualquer outra tarefa **não iniciada** de prioridade **maior**. Alta bloqueia Média e Baixa; Média bloqueia Baixa.
2. **Dentro do mesmo nível — só pra Alta e Média:** tem que iniciar a **mais antiga** primeiro (FIFO por `createdAt`). Se existirem duas tarefas Alta não iniciadas, a mais nova fica bloqueada até a mais antiga ser iniciada.
3. **Baixa não tem regra de ordem entre si** — várias tarefas Baixa podem ser iniciadas em qualquer ordem, desde que não haja Alta/Média pendente (regra 1 ainda vale).
...
O quadro mostra uma faixa vermelha no topo com a tarefa de prioridade Alta/Média mais antiga ainda não iniciada (`findMostUrgentPending` em `src/lib/priority.ts`) — visível sempre, não só quando alguém tenta furar a fila.
...
- Duas tarefas Alta: iniciar a mais nova é bloqueado; iniciar a mais antiga libera; depois a mais nova pode ser iniciada.
- Tarefa Baixa é bloqueada enquanto existir Alta pendente, mesmo que a Alta tenha sido criada depois da Baixa.
