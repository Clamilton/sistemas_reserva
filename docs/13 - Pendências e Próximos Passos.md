...
- [ ] **Em aberto:** PVA rejeitava o `M105`/`M505` que a ferramenta gera pro crédito extemporâneo ("Não deverá existir um registro M105 ... para Código da Natureza da Base de Cálculo do Crédito e Código de Situação Tributária não informados nos documentos e operações"), e depois, com outra empresa, rejeitou de novo por "Duplicidade de ocorrência da chave" (causada pelo placeholder `M100|101`/`M500|101` zerado colidindo com um `101` real do contribuinte). O placeholder `101` foi removido — agora só o `M100|201`/`M500|201` dedicado (com o crédito real) é gerado, o que elimina a duplicidade de chave estruturalmente. **O que ainda não foi confirmado**: se esse `M100|201`/`M105` dedicado, sozinho, passa no PVA sem o erro original de `NAT_BC_CRED=13`/`CST=53` "não informado nos documentos" — só um novo teste do usuário no validador da Receita confirma. Se persistir, a pergunta de fundo continua a mesma: existe um código de natureza/CST correto pra crédito extemporâneo (sem exigir documento em Bloco C/D)? Ver detalhe em [[16 - SPED Retificador]].
