...
- [ ] **Em aberto:** quatro rodadas de correção no lançamento do crédito extemporâneo em M100/M500 (contaminação de crédito real → duplicidade de chave → código errado numa empresa com `101` real → inconsistência com o Bloco 1 numa empresa sem M100/M500 nenhum), cada uma motivada por um teste real no validador da Receita, em três empresas diferentes — ver a sequência completa em [[16 - SPED Retificador]]. A versão atual: usa `101` se existir, senão `201` se existir, senão cria um par novo em `201` (consistente com o Bloco 1, que sempre usa `201`) — mas essa versão específica **ainda não foi testada** no validador. Confirmar no próximo SPED gerado se chega a 0 erros.
