...
- [ ] **Em aberto:** três rodadas de correção no lançamento do crédito extemporâneo em M100/M500 (contaminação de crédito real → duplicidade de chave → código errado), cada uma motivada por um teste real no validador da Receita — ver a sequência completa em [[16 - SPED Retificador]]. A versão atual soma o crédito direto no `M100|101`/`M500|101` existente (grupo confirmado pelo próprio PVA), incluindo `M105`/`M505|13|53` no mesmo grupo. Isso já derrubou o total de erros de 12 pra 2 no último teste — mas essa versão específica **ainda não foi testada** no validador. Confirmar no próximo SPED gerado se chega a 0 erros.
