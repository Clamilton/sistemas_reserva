...
- [x] **Resolvido:** seis rodadas de correção no lançamento do crédito extemporâneo em M100/M500, em três empresas diferentes — ver a sequência completa em [[16 - SPED Retificador]]. Desenho final: o `101` é declaratório (nunca soma no pai) e o `201` carrega o crédito, mas **os dois sempre aparecem** no arquivo; se já existe qualquer `101` (mesmo de alíquota fora da padrão), ele só recebe o `M105`/`M505` e o placeholder não é emitido — é essa guarda que evita a duplicidade de chave; `1100`/`1500` sempre usa `201`, filtrando por período. **Confirmado pelo usuário no PVA**, primeira versão validada de ponta a ponta.
- [ ] Com o bloco M vazio (`M001` com `IND_MOV=1`), a ferramenta abre o bloco e insere `M100`/`M105`/`M500`/`M505`, mas **não gera `M200`/`M600`** — que são obrigatórios num bloco M com movimento. Não é regressão: o `sped_retificador.py` de referência tem o mesmo comportamento, e nenhum arquivo real testado até hoje caiu nesse caminho. Afeta empresa no regime cumulativo que precise de retificação. Resolver quando aparecer o primeiro caso — ou preventivamente, se for barato.
- [ ] O `F_SAIR_GRUPO` do bloco F já errou duas vezes em direções opostas (item 23 curto demais, depois largo demais — ver [[12 - Histórico de Decisões]]). Hoje está conferido contra o leiaute oficial, mas não há teste automatizado que trave a regra: qualquer mexida futura nesse conjunto depende de alguém lembrar da distinção irmão/filho. Vale um teste de regressão se o módulo continuar recebendo manutenção.
