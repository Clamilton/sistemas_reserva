...
> 3. **Código errado (empresa com crédito 101 real)**: removido o placeholder `101` de vez (nunca carregou crédito, só duplicava a base sob um código redundante com o `201`) — sobrou só o par `201`, dedicado, sem duplicidade. Mesmo assim o PVA voltou a pedir, explicitamente: *"Deverá existir um registro M105 ... (COD_CRED = 101, ..., NAT_BC_CRED = 13, CST_PIS = 53)"*. O validador estava dizendo, sem ambiguidade, que esse detalhamento pertence ao grupo `101` — o crédito normal do contribuinte —, não a um `201` isolado. Corrigido pra somar o crédito extemporâneo direto no `M100|101`/`M500|101` existente — base e crédito também no **próprio registro pai** (antes ficavam zerados no placeholder, a provável causa real do erro 1) e no `M105`/`M505|13|53` filho. Sem nenhum `101` prévio, criava um par novo — usando `101`.
> 4. **Inconsistência com o Bloco 1 (empresa sem nenhum M100/M500)**: numa terceira empresa, com Bloco M **completamente vazio** (nem `101` nem `201`), o fix do problema 3 criava o par novo em `101` — mas o Bloco 1 (`1100`/`1500`, registros de controle que também carregam o crédito) sempre usa `201`, sem exceção. PVA cruzou os dois blocos e pediu, de novo explicitamente: *"Deverá existir um registro M100 ... (COD_CRED = 201, ..., NAT_BC_CRED = 13, CST_PIS = 53)"* — dessa vez pedindo `201`, não `101`.
>
> **Correção atual**: busca um `M100`/`M500` existente com `COD_CRED=101` primeiro (o caso mais comum — crédito vinculado a receita tributada); se não achar, tenta `201`; só cria um par novo — usando `201`, pra ficar consistente com o Bloco 1 — se nenhum dos dois existir. Onde quer que o crédito entre, soma base+crédito no próprio `M100`/`M500` pai (não só no `M105`/`M505` filho). Validado localmente (fora do PVA): SPED com `101` real (soma nele, sem regressão) e SPED sintético com Bloco M vazio (cria par novo em `201`, consistente com o `1100`/`1500` do mesmo arquivo). **Ainda sem confirmação de 0 erros no PVA com esta versão específica** — depende do próximo teste do usuário.
