...
> [!bug] Crédito extemporâneo em M100/M500: cinco tentativas até chegar no desenho certo
...
>
> **Desenho final**: as duas coisas acontecem de forma independente. Se existir um `M100|101`/`M500|101` (aliq padrão), ganha só um `M105`/`M505|13|53` com a base declarada e crédito zero — pai intocado. Em paralelo, **sempre** existe um `M100|201`/`M500|201` com o crédito de verdade — soma no existente (achando pelo marcador `M105|13|53`) ou cria um par novo, pai e filho totalmente atualizados. O Bloco 1 (`1100`/`1500`) sempre usa `201`, e agora filtra por **período** (`PER_APU_CRED`) além do código — um SPED real tinha um `1100|101` de um mês anterior que, sem esse filtro, absorvia o crédito do mês sendo retificado (`achar1100Periodo()`).
>
> Validado localmente contra os dois SPEDs reais com `101` legítimo (um com 5 naturezas, outro com 1, esse último também com `1100`/`1500` de um período anterior): o `101` real fica intocado (só ganha o `M105` informativo, `VL_BC_PIS=0`), o `201` novo carrega o crédito completo, e o `1100`/`1500` do período anterior não é contaminado. **Ainda sem confirmação de 0 erros no PVA com esta versão** — depende do próximo teste do usuário.
