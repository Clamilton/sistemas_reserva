...
>
> **Essa expansão foi longe demais e precisou ser corrigida depois** — "todo registro que vem depois do F100" incluiu registros-**filho**, e o `F111` virou ponto de inserção. Ver o callout logo abaixo.

> [!bug] O `F100` novo roubava o `F111` do contribuinte
> Consequência direta do fix acima. O conjunto de "saída do grupo" passou a incluir `F111`, `F129`, `F139`, `F205`, `F210`, `F211`, `F509`, `F519`, `F559`, `F569` — mas esses não são irmãos do `F100`, são **filhos** de outros registros, e no SPED o vínculo pai-filho é **posicional**: um `F111` pertence ao `F100` imediatamente anterior a ele.
>
> Num arquivo com `F010 > F100(existente) > F111`, o `F100` novo entrava **entre os dois**. O processo referenciado do contribuinte passava a pertencer ao nosso `F100` de recuperação de crédito, e o `F100` original ficava sem ele — corrupção silenciosa de dado do contribuinte, não só ordem trocada.
>
> Na prática só o `F111` chegava a ser testado: é o único filho que aparece antes de qualquer irmão de `F100` (os outros vêm depois de `F120`/`F130`/`F200`/`F500`/`F510`/`F550`/`F560`, que já são pontos de saída legítimos e sempre casam primeiro). Os demais eram inofensivos, mas errados pela mesma regra.
>
> Corrigido reduzindo `F_SAIR_GRUPO` só aos **irmãos** de `F100` mais os delimitadores: `F010`, `F120`, `F130`, `F150`, `F200`, `F500`, `F510`, `F525`, `F550`, `F560`, `F600`, `F700`, `F800`, `F990`. A parte legítima do fix anterior (`F120`/`F130`/`F150`, que de fato precisam vir depois do `F100`) continua valendo — o cenário do `F120` que originou aquele callout segue passando.
...
> [!bug] O aviso da tela dizia que usa o primeiro `0140`, não a matriz
> Quando o SPED tem mais de um `0140`, a página emite um aviso no log. O texto era herdado da versão Python: *"AVISO: arquivo tem N estabelecimentos (registros 0140). Sempre é usado o CNPJ do PRIMEIRO — confira se é o correto"*, em vermelho — e nunca foi atualizado quando `info0140()` passou a preferir a matriz.
>
> Efeito prático: num arquivo com matriz e filial, o usuário era mandado conferir o estabelecimento **errado**, e um arquivo processado corretamente aparecia como erro no log. Não quebrava o arquivo gerado, mas destruía a confiança no que a ferramenta estava fazendo — justamente no cenário que a regra da matriz existe pra resolver.
>
> Corrigido: quando há matriz entre os `0140`, o log informa qual foi usada (nome + CNPJ) como mensagem normal; o vermelho fica só pro caso que de fato pede atenção — nenhum `0140` com CNPJ de matriz, caindo no fallback do primeiro registro. `ehMatriz()` passou a ser exportado do `parser.ts` pra UI usar exatamente a mesma regra do gerador, em vez de reimplementar o teste de CNPJ.
>
> Lição que vale além desse caso: quando uma regra de negócio muda, o texto que **explica** a regra pro usuário é parte da mudança. Um aviso desatualizado mente com a mesma autoridade de um correto.

> [!bug] Crédito extemporâneo em M100/M500: seis tentativas até chegar no desenho certo
> A porta original (fiel ao Python) lançava o crédito extemporâneo em dois registros por tributo: um `M100|101`/`M500|101` (código do crédito "normal", vinculado a receita tributada) e um `M100|201`/`M500|201` (vinculado a receita não tributada) — cada um com seu `M105`/`M505|13|53` (natureza "outras operações", CST 53, descrição "RECUPERACAO DE CREDITOS TRIBUTARIOS"). Sempre buscava um `M100`/`M500` existente com o mesmo código pra "aproveitar" antes de criar um novo. Cinco problemas reais, cada um só apareceu testando contra uma empresa de produção diferente (todo teste anterior tinha Bloco M vazio, onde nenhum desses problemas se manifesta):
>
> 1. **Contaminação de crédito real**: a busca por um `M100`/`M500` existente não distinguia "nosso" (de uma rodada anterior da ferramenta) de um crédito **real e legítimo** do contribuinte sob o mesmo código (ex.: `101` — Aquisição de Bens para Revenda, presente em quase todo SPED com Bloco M preenchido). Um SPED real com `M100|101` genuíno ganhou um `M105|13|53` anexado — PVA rejeitou: *"Não deverá existir um registro M105 ... não informados nos documentos e operações"*.
> 2. **Chave duplicada**: a correção seguinte passou a sempre criar um par **novo e dedicado** — inclusive pro `101` — quando não achava o marcador "nosso". Só que `101` sempre usa a alíquota padrão, a mesma de qualquer `101` real — duas linhas com a mesma chave (`COD_CRED`+`ALIQ`) violam a unicidade do SPED. PVA rejeitou com *"Duplicidade de ocorrência da chave"*.
> 3. **Tentativa de unificar num código só**: removido o `101` fixo, testado só com `201` dedicado — o PVA voltou a pedir explicitamente um `M105` em `COD_CRED = 101`. Tentativa seguinte: reintroduzido o `101`, mas escolhendo **um único código pra tudo** (`101` se já existisse no arquivo, senão `201`) — incluindo somar base **e crédito** no próprio `M100`/`M500` pai, não só no `M105` filho.
> 4. **O `201` some quando existe um `101` real**: com a lógica de "um código só", numa empresa que já tinha um `M100|101` real (o caso mais comum), o `201` nunca era criado — e o PVA reclamou, agora pedindo explicitamente `M100 ... COD_CRED = 201`. As duas coisas nunca foram alternativas uma da outra: o crédito de verdade *sempre* precisa existir em `201`, exista ou não um `101`.
> 5. **A correção final do usuário**: "não pode somar no M100 quando é código 101, só adiciona a base no M105". Ou seja, mesmo com `201` sempre presente separadamente, o `101` (quando existe) nunca pode ter base/crédito somados no próprio registro pai — só ganha um `M105`/`M505|13|53` novo com a base em TOT/NC e **`VL_BC_PIS`/`COFINS` em 0** (nenhum crédito alocado ali). Essa é, sim, a mesma forma do `linhaM105_101` original do Python — só que agora buscando o `101` real certo (sem duplicar chave) e sem nunca tocar no pai.
> 6. **O `101` sumiu do arquivo**: a remoção do placeholder (tentativa 2) tinha sido **total**. Sobrou só o caminho "se já existe um `101`, anexa o `M105` nele" — então todo arquivo **sem** nenhum `M100|101` passou a sair sem o código 101, silenciosamente. O diagnóstico da duplicidade de chave estava certo; a amputação é que foi larga demais. Agravante: `acharM100()` filtra por alíquota, então um `101` real com alíquota fora da padrão (empresa no cumulativo, 0,65%/3%) não era encontrado — e ali a versão antiga *inseria* um segundo `101`, que é exatamente a duplicidade que se queria evitar.
>
> **Desenho final**: as duas coisas acontecem de forma independente. O `101` é **declaratório** (nunca soma crédito no pai, nunca aloca `VL_BC`) e o `201` é quem **carrega o crédito** — mas os dois sempre precisam aparecer no arquivo.
>
> - **`101`** — se existir **qualquer** `M100|101`/`M500|101` no arquivo (a busca cai pra `acharM100(..., '101', null)`, ignorando a alíquota, quando não acha com a padrão), ele recebe só um `M105`/`M505|13|53` com a base declarada e crédito zero, pai intocado. Se não existir nenhum, aí sim é emitido o par placeholder zerado (`linhaM100_101`/`linhaM500_101` + `M105`/`M505`). Nunca as duas coisas — é essa guarda que impede a duplicidade da chave `COD_CRED`+`IND_CRED_ORI`+`ALIQ`+`ALIQ_QUANT`.
> - **`201`** — **sempre** existe, com o crédito de verdade: soma no existente (achando pelo marcador `M105|13|53`) ou cria um par novo, pai e filho totalmente atualizados.
> - **Bloco 1** (`1100`/`1500`) sempre usa `201`, e filtra por **período** (`PER_APU_CRED`) além do código — um SPED real tinha um `1100|101` de um mês anterior que, sem esse filtro, absorvia o crédito do mês sendo retificado (`achar1100Periodo()`).
>
> Validado rodando `buildSped()` contra 11 SPEDs sintéticos: 7 de matriz/filial (filial listada antes da matriz, matriz sem `F010` no bloco F, bloco F vazio, só filiais no `0140`, `F120`/`F129` depois do `F100`) e 4 de bloco M (sem nenhum `101`; bloco M inteiramente vazio; `101` com alíquota fora da padrão; `101` com alíquota padrão). Em todos: `101` presente, zero duplicidade de chave, `F100` na posição certa. O cenário que já tinha sido aprovado no PVA (`101` com alíquota padrão) sai **byte a byte igual** ao de antes — a mudança só acrescenta comportamento onde antes não havia nenhum.
>
> ✅ **Confirmado pelo usuário no PVA** — depois de seis rodadas e três empresas, essa é a primeira versão com validação de ponta a ponta no validador da Receita.
