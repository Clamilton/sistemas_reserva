...
> [!bug] M105/M505 novo podia sair anexado a um crédito real do contribuinte, e mesmo dedicado o PVA ainda rejeita (em aberto)
> Pra lançar o crédito extemporâneo, a ferramenta sempre tenta achar um `M100`/`M500` já existente com o mesmo `COD_CRED` (`101` ou `201`) e mesma alíquota pra "aproveitar" — pensado pra quando a própria ferramenta já rodou antes sobre esse SPED (reaproveitar o que ela mesma criou numa rodada anterior). O problema: a busca (`acharM100`) não distinguia isso de um `M100|101` **real e legítimo** do contribuinte (ex.: "101 — Aquisição de Bens para Revenda", presente em praticamente todo SPED com Bloco M preenchido) — testado com um SPED real que já tinha `M100|101` com créditos genuínos (`NAT_BC_CRED` 01/02/03/04/09), a ferramenta anexou um `M105|13|53` novo como se fosse mais um detalhe desse crédito real, sem nenhum documento por trás. Corrigido: só reaproveita um `M100`/`M500` achado se ele já tiver o filho `M105`/`M505|13|53` que só esta ferramenta cria (o marcador de "nosso") — sem esse filho, sempre insere um par novo e dedicado, nunca anexado a um registro alheio.
>
> Isso resolveu a contaminação do `M100|101` real, mas **não resolveu os erros do PVA** — testando contra o mesmo arquivo, tanto o par `101` (placeholder zerado) quanto o `201` (crédito real), mesmo cada um em seu próprio `M100`/`M500` dedicado (nunca existiu `M100|201` nesse arquivo antes, então esse par já era "novo" mesmo antes do fix), continuam sendo rejeitados: *"Não deverá existir um registro M105 ... para Código da Natureza da Base de Cálculo do Crédito e Código de Situação Tributária não informados nos documentos e operações"*. Ou seja, o problema não é "anexado a um registro errado" — é que o `NAT_BC_CRED=13`/`CST=53` usado (com a descrição "RECUPERACAO DE CREDITOS TRIBUTARIOS") não tem nenhum documento no Bloco C/D dessa competência que o sustente, e o PVA cruza essa informação. Faz sentido: o crédito vem de um Acórdão (decisão judicial), não de uma nota fiscal do mês. **Em aberto** — precisa de orientação de quem opera a ferramenta sobre o código de natureza/CST correto pra esse tipo de crédito, ou se o placeholder `101` deveria simplesmente parar de ser gerado. Ver [[13 - Pendências e Próximos Passos]].

> [!bug] Placeholder `M100|101`/`M500|101` zerado causava "Duplicidade de ocorrência da chave"
> Reportado com outro SPED real (de outra empresa, Bloco M já com `M100|101`/`M500|101` legítimos e com crédito de verdade do contribuinte). Como o fix acima passou a **sempre** criar um par novo e dedicado quando não achava "nosso" marcador — inclusive pro placeholder `101` — a ferramenta criou um **segundo** `M100|101`/`M500|101`. O problema: o placeholder sempre usa a alíquota padrão (1,65%/7,60%), a mesma de praticamente qualquer `M100|101`/`M500|101` real — duas linhas com a mesma chave (`COD_CRED`+`IND_CRED_ORI`+`ALIQ`+`ALIQ_QUANT`) violam a unicidade do SPED. O PVA rejeitou com *"Duplicidade de ocorrência da chave"*, e isso ainda arrastou erro pros `M105`/`M505` **reais** desse mesmo grupo (*"Deverá existir um registro M105 ... informados nos documentos"* — o oposto do erro anterior, "não deverá existir"), provavelmente porque a chave duplicada confunde o agrupamento que o validador faz entre `M100` e seus `M105` filhos.
>
> Reconsiderado: o placeholder `101` nunca carregou crédito nenhum (`VL_CRED_PIS`/`COFINS` sempre 0) — sua única função era declarar a mesma base do crédito extemporâneo sob um código que "não aloca crédito", redundante com o `M100|201` (que já carrega o crédito real). Removido de vez — `linhaM100_101`/`linhaM105_101`/`linhaM500_101`/`linhaM505_101` e toda a lógica de inserção associada saíram do código. Só o par `M100|201`/`M105` e `M500|201`/`M505` (dedicado, nunca anexado a um registro alheio) continua sendo gerado. Validado contra o SPED que já tinha `M100|101`/`M500|101` reais: sem duplicidade de chave, os registros reais saem intocados, e exatamente 1 par `201` novo carrega o crédito.
>
> **Ainda em aberto**: essa mudança elimina estruturalmente a duplicidade de chave, mas não confirma se o `NAT_BC_CRED=13`/`CST=53` do `M105`/`M505` dedicado passa no PVA sem exigir documento em Bloco C/D (a pergunta do bug anterior) — só um novo teste do usuário no validador da Receita confirma. Ver [[13 - Pendências e Próximos Passos]].
