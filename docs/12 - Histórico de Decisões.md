...
## 33. SPED Retificador: fallback pra COD_CRED=201 quando não existe M100/M500 nenhum

Usuário testou o fix do item 32 numa TERCEIRA empresa (AUTO POSTO TOURAO LTDA) — 2 erros, mas de um tipo novo: *"Deverá existir um registro M100 ... (COD_CRED = 201, ..., NAT_BC_CRED = 13, CST_PIS = 53)"*, apontando pro registro `M001|0|` (ou seja: essa empresa não tinha NENHUM `M100`/`M500` no Bloco M, nem `101` nem `201` — Bloco M inteiramente vazio antes da retificação).

Causa: o fix do item 32 sempre criava o par novo em `101` quando não achava nenhum M100/M500 existente. Só que o Bloco 1 (`1100`/`1500` — registros de controle de créditos fiscais, que também carregam o mesmo crédito) **sempre** usa `COD_CRED=201`, desde a porta original do Python, e nunca foi alterado nas correções anteriores. Com Bloco M em `101` e Bloco 1 em `201`, os dois blocos ficaram inconsistentes — e o PVA cruza essa informação entre eles.

Corrigido: busca `M100`/`M500` com `101` primeiro (o caso mais comum, confirmado no item 32); se não achar, tenta `201`; só cria um par novo — agora em `201`, pra bater com o Bloco 1 — se nenhum dos dois existir. De quebra, o teste local pego um bug introduzido no item 32: ao inserir o `M105`/`M505` novo num grupo M100/M500 já existente mas sem esse filho ainda, o código sempre gerava `M105` (mesmo no lado do `M500`/COFINS) — sem efeito visível nos testes anteriores porque nenhum deles exercitava esse ramo especificamente pro M505, só apareceu rodando a soma completa de novo depois desta mudança.

Validado localmente: SPED com `101` real (soma nele, sem regressão) e SPED sintético com Bloco M vazio (cria par novo em `201`, consistente com o `1100`/`1500` gerado no mesmo arquivo). Detalhe técnico completo (as quatro tentativas em sequência, três empresas diferentes) em [[16 - SPED Retificador]]. **Ainda sem confirmação de 0 erros no PVA** — depende do próximo teste do usuário.

## 34. SPED Retificador: 101 nunca soma no M100/M500, 201 sempre existe separado

Usuário mandou o SPED real da terceira empresa (item 33) dessa vez, não só o relatório de erros — e junto, uma correção direta e urgente: *"tu tá somando no M100 quando é codigo 101, não pode, somente adiciona a base no M105"*.

Lendo o arquivo real (coisa que só foi possível porque o usuário anexou dessa vez): a empresa **tinha**, sim, um `M100|101` real (com crédito genuíno, `NAT_BC_CRED` 04) — a suposição do item 33 de que o Bloco M estava "completamente vazio" (baseada só no relatório de erros citando `|M001|0|` como contexto) estava **errada**. O item 33 tinha corrigido a inconsistência com o Bloco 1, mas manteve o problema do item 32: escolher **um único código pra tudo** (`101` OU `201`, nunca os dois) — como essa empresa tinha `101` real, a ferramenta usava só `101` (inclusive somando base e crédito nele, contra a regra que o usuário acabou de apontar) e nunca criava o `201` — daí o erro pedindo explicitamente `COD_CRED = 201`.

Corrigido o desenho pra valer: as duas coisas são **independentes**, não uma escolha exclusiva.
- Se existir um `M100|101`/`M500|101` real (aliq padrão): ganha só um `M105`/`M505|13|53` novo com a base em TOT/NC — **crédito zero**, pai nunca tocado. Essa é a mesma forma do `linhaM105_101` que existia desde a porta original do Python, só que agora aplicada ao `101` real certo (achado sem duplicar chave, graças aos fixes anteriores).
- **Sempre** existe também um `M100|201`/`M500|201` com o crédito de verdade — soma no existente (achado pelo marcador `M105|13|53`, evitando contaminar um `201` real e desconhecido) ou cria um par novo, pai e filho totalmente atualizados.
